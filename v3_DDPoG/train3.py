...
class agent:
    ...
    def actor_loss(self, probs, actions, adv, old_probs, closs):
        ...
        entropy = tf.reduce_mean(
            tf.math.negative(tf.math.multiply(probability, tf.math.log(probability)))
        )
        # print(probability)
        # print(entropy)
        sur1 = []
        ...
        for pb, t, op, a in zip(probability, adv, old_probs, actions):
            ...
            # print(f"s1{s1}")
            s2 = tf.math.multiply(
                tf.clip_by_value(ratio, 1.0 - self.clip_pram, 1.0 + self.clip_pram),
                t,
            )
            # print(f"s2{s2}")
            sur1.append(s1)
        ...
        loss = tf.math.negative(
            tf.reduce_mean(tf.math.minimum(sr1, sr2)) - closs + 0.001 * entropy
        )
        # print(loss)
        return loss
...
for s in range(steps):
    ...
    states, actions, returns, adv = preprocess1(
        states, actions, rewards, dones, values, 1
    )
    ...
    if avg_reward > best_reward:
        ...
        # agentoo7.critic.save(
        #     "model_critic_{}_{}".format(s, avg_reward), save_format="tf"
        # )
        best_reward = avg_reward
    if best_reward > env.reward_goal:
        target = True
    env.reset()
